...
> ✅ **Resolved (2026-08-31), HPC ticket RIV-20794:** Before this fix, Globus transfers of any object archived in Glacier Flexible Retrieval simply failed — the object had to be restored first, and restores were handled by hand. We asked Research Computing to enable Globus's built-in `--s3-restore` support on the `dremel-lab-bucket` storage gateway instead, configured with `restore_tier = Standard` (~3-5 hour retrieval) and `restore_days = 5-7` (so a restored copy stays available long enough for large or multi-file transfers), after confirming the AWS credentials behind the collection had the `s3:RestoreObject` IAM permission. RC (Michele Co) enabled it on the gateway config (`S3 Restore Archived Objects: True`); our real-world test transfer completed successfully, and the ticket was closed. Manually emailing Sarah Dremel to restore a Glacier object is **no longer necessary**.

### How to Download Glacier Files

Just start the download/transfer in Globus as normal — no email or manual request required:

1. **Select the Glacier file** and start the download/transfer in Globus as you would any other file
2. **Globus detects the object is archived** and automatically submits an AWS restore request on your behalf
3. **Wait for the restore to complete** — configured for **Standard tier**, typically **3-5 hours**
4. **Globus finishes the transfer automatically** once the object is restored — you don't need to retry or come back to re-trigger anything
5. The restored copy stays available for **5-7 days** (`restore_days`), so it's fine if a large multi-file transfer takes a while to work through the queue
...
↓
Restored copy remains available for 5-7 days if you need it again
```

### Important Notes

> ⚠️ **Plan ahead:** Restoration still takes a few hours (Standard tier, ~3-5 hours), so a Glacier download won't complete instantly — kick off large/Glacier transfers before you need the files.
> ⚠️ **Large or multi-file transfers:** Restored copies stay available for 5-7 days, which gives Globus enough time to work through big batches without files expiring mid-transfer.
> ℹ️ **If a Glacier transfer still fails:** This used to require emailing Sarah Dremel for a manual restore. Since the fix above, a failure more likely means a permissions or connector issue — report it in `#dremellab` or to Research Computing rather than requesting a manual restore.
...
1. Just start the transfer in Globus as normal
2. Globus automatically submits the restore request and waits; expect roughly **3-5 hours** (Standard tier) before the transfer completes
3. No action needed on your part — do not email for a manual restore, this is now automatic
